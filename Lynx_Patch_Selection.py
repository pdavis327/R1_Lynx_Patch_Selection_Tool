...
class Lynx_Patch_Selection(object):
	def __init__(self):
		...
		self.label = "Lynx Patch Selection"
		self.description = "This tool selects contiguous raster areas greater than or equal to a user defined agreage based on a habitat suitability raster class. The selection is exported to a new polygon file, and each feature is attributed with the proportion and area of a high suitability raster. Optionally, the patches can be buffered. In this case the option to compute low suitability proportion and area becomes availalble. Additionally, if there are donut holes in the polygons, acreage of another raster can be computed within those areas for each polygon."
	...
	def execute(self, parameters, messages):
		...
		if opt_buffer:
			arcpy.AddMessage("\nBuffering patch.")
			Mod_temp_buff = arcpy.Buffer_analysis(Mod_Ras_1_sel_poly, r"in_memory\Mod_temp_buff", buff_dis, "", "", "ALL")
			Mod_Ras_1_sel_poly = arcpy.MultipartToSinglepart_management(Mod_temp_buff, r"in_memory\rasToPoly_buff_sing")
			addField(Mod_Ras_1_sel_poly, "ID", "SHORT")
			
			rec = 0
			with arcpy.da.UpdateCursor(Mod_Ras_1_sel_poly, "ID") as cursor:
				for row in cursor:
					if row[0] == 0:
						rec = 1
					else:
		...
		addAreaField(Mod_temp, "Patch_Acres")
		arcpy.AddMessage("\nSelecting patches >= {}-acres in {} class {}.\n".format(acreage, Mod_Ras_1, Mod_Ras_1_Val))
		arcpy.SelectLayerByAttribute_management(Mod_temp, "", ' "Patch_Acres" >= {} '.format(acreage))	
		
		#Isolate donut holes
		if opt_buffer or Donut_1:
		...
		if opt_buffer:
			Mod_temp = arcpy.Erase_analysis(Mod_temp, Donuts, r"in_memory\Mod_buff_elim_erase")
			arcpy.CalculateField_management(Mod_temp, "Patch_Acres", "!SHAPE.area@ACRES!", "PYTHON_9.3")	
		...
		identFeat_stats = arcpy.Statistics_analysis(identFeat, r"in_memory\identFeat_stats", 'Patch_Acres MEAN;High_Acres SUM', 'ID')		
		...
		arcpy.CalculateField_management(identFeat_stats, 'HIGH_PERC', '[HIGH_ACRES]/ [MEAN_PATCH_ACRES]', 'VB')		
		...
		if Low_Ras_3:
			...
			Low_Ras_3_sel_poly = arcpy.MultipartToSinglepart_management(Low_Ras_3_sel_poly, r"in_memory\Low_Ras_3_sel_poly_clip_sing")
			...
			identFeat2_stats = arcpy.Statistics_analysis(identFeat2, r"in_memory\identFeat_stats2", 'Patch_Acres MEAN;Low_Acres SUM', 'ID')	
			...
			arcpy.CalculateField_management(identFeat2_stats, 'LOW_PERC', '[LOW_ACRES]/ [MEAN_PATCH_ACRES]', 'VB')
		...
		arcpy.DeleteField_management(Mod_temp, "gridcode")
